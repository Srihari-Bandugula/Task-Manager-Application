...
const updateTaskChecklist = async(req,res) =>{
  try{
    ...
    //auto- ipdate progrress based on checklist completion
    const completedCount = task.todoChecklist.filter(
      (item) => item.completed
    ).length;
...
const shareTask = async (req, res) => {
  try {
    ...
    if (task.assignedTo.includes(userIdToShareWith)) {
      return res.status(400).json({ message: "User already assigned to this task" });
    }

    task.assignedTo.push(userIdToShareWith);
    await task.save();
...
const getUserDashboardData = async(req,res) =>{
  try{
    ...
    // Fetch statastics for user-specific tasks
